    ...
    for t = 0.1:0.1:1.9
        for w = 0.1:0.1:1.9
            ...
            while(itcount < max_interations)
                temp = zeros(n,1); temp2 = zeros(n,1);              
                for i = 1:n % iterative method using components
                    if i > 2 && i <= n-2
                        Ux(i) = dot(-A(i,i+1:i+2)/A(i,i),solution(i+1:i+2)'); % Niethammer's Scheme
                    ...
                    elseif i == n-1
                        Ux(i) = dot(-A(i,n)/A(i,i),solution(n)); % Niethammer's Scheme
                        temp2(i) = (1-t)*solution(i) + w*dot(-A(i,i-2:i-1)/A(i,i), temp2(i-2:i-1)') + (t-w)*dot(-A(i,i-2:i-1)/A(i,i),solution(i-2:i-1)') + t*Ux(i) + (t*b(i)/A(i,i));
                    else
                        Ux(i) = 0; % Niethammer's Scheme
